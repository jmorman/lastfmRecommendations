...
class MRCalculateSimilarityTrimmedB(MRJob):
	...
	def reducer_artist_similarities(self, artist_pair, rating_pairs):
		...
		if(len(rating_pairs) > 2):
			...
			score = pearsonr(artist1_ll_minus_ave, artist2_ll_minus_ave)[0]
			#Apply-regularization-later version:
			if(score >= 0):
				yield (artist_pair, score*len(rating_pairs)/(len(rating_pairs)+5))

if __name__ == '__main__':
	MRCalculateSimilarityTrimmedB.run()
