...
class MRCalculateSimilarityB(MRJob):
	...
	def reducer_artist_similarities(self, artist_pair, rating_pairs):
		...
		if(len(rating_pairs) > 1):
			...
			#Baked-in-regularization version:
			yield (artist_pair, len(rating_pairs) * pearsonr(artist1_ll_minus_ave, artist2_ll_minus_ave)[0] 
									/ (len(rating_pairs) + 3))

if __name__ == '__main__':
	MRCalculateSimilarityB.run()
